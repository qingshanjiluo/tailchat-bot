...
class ActiveMessageSender:
    ...
    def _register_scheduled_tasks(self):
        ...
        # 每天早上8点发送问候
        self.scheduler.every().day.at("08:00").do(self._send_morning_greeting)

        # 每天中午12点发送午间问候
        self.scheduler.every().day.at("12:00").do(self._send_noon_greeting)

        # 每天晚上20点发送晚间问候
        self.scheduler.every().day.at("20:00").do(self._send_evening_greeting)

        # 每小时发送小提示（随机时间）
        self.scheduler.every().hour.do(self._send_random_tip)

        # 每2小时发送趣味知识（随机时间）
        self.scheduler.every(2).hours.do(self._send_fun_fact)

        # 每天检查活跃用户并发送个性化消息
        self.scheduler.every().day.at("10:00").do(self._check_active_users)

        # 每周一发送周报提醒
        self.scheduler.every().monday.at("09:00").do(self._send_weekly_reminder)
    ...
    def send_to_converse(
        self, converse_id: str, message: str, mentions: List[str] = None
    ):
        """发送消息到指定会话"""
        if not self._is_active_hours():
    ...
    def send_ai_generated_message(
        self, topic: str, target_converse_id: Optional[str] = None
    ):
        """发送AI生成的消息"""
        if not self._is_active_hours():
    ...
    def get_schedule_info(self) -> Dict:
        ...
        return {"running": self.running, "job_count": len(jobs), "jobs": jobs}
