...
class TailChatBrowserClient:
    ...
    async def login(self) -> bool:
        ...
        try:
            ...
            try:
                # 尝试查找用户头像或已登录标识
                avatar_selector = 'img[src*="avatar"], .user-avatar, [data-testid="user-avatar"]'
            ...
            try:
                # 等待用户信息加载
                await self.page.wait_for_selector('img[src*="avatar"], .user-avatar, [data-testid="user-avatar"]', timeout=10000)
                logger.info("登录成功")
    ...
    async def _get_user_info(self):
        ...
        try:
            ...
            if not self.user_id:
                try:
                    user_element = await self.page.wait_for_selector('[data-user-id], [data-userid], .user-id', timeout=2000)
                    if user_element:
                        self.user_id = await user_element.get_attribute('data-user-id') or \
                                      await user_element.get_attribute('data-userid') or \
                                      await user_element.text_content()
                except:
                    pass
    ...
    async def _ensure_message_page(self):
        ...
        try:
            # 检查是否在消息页面
            current_url = self.page.url
            if '/chat' in current_url or '/message' in current_url or '/converse' in current_url:
                return True
    ...
    async def _extract_message_data(self, element) -> Optional[Dict]:
        ...
        try:
            # 获取消息ID
            message_id = await element.get_attribute('data-message-id') or \
                        await element.get_attribute('id') or \
                        str(time.time())
            
            # 获取消息内容
            content_element = await element.query_selector('.message-content, .content, [class*="content"]')
            content = await content_element.text_content() if content_element else ""
            
            # 获取作者信息
            author_element = await element.query_selector('.message-author, .author, [class*="author"]')
            author = await author_element.text_content() if author_element else ""
            author_id = author  # 简化处理
            
            # 检查是否是私信
            is_dm = False
            dm_indicators = await element.query_selector_all('[class*="dm"], [class*="private"], [data-type="dm"]')
            if dm_indicators:
                is_dm = True
            
            # 检查@提及
            mentions = []
            mention_elements = await element.query_selector_all('.mention, [data-mention]')
            for mention_element in mention_elements:
                mention_id = await mention_element.get_attribute('data-mention') or \
                            await mention_element.get_attribute('data-user-id')
                if mention_id:
            ...
            return {
                ...
                'author': author_id.strip() if author_id else '',  # TailChat中author就是用户ID
    ...
    async def send_message(self, converse_id: str, content: str, mentions: List[str] = None):
        """发送消息"""
        try:
            ...
            for selector in input_selectors:
                try:
                    message_input = await self.page.wait_for_selector(selector, timeout=2000)
                    if message_input:
                        break
                except:
                    continue
