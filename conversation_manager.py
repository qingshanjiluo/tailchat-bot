...
class ConversationManager:
    ...
    def get_or_create_conversation(self, converse_id: str, user_id: str, user_name: str, is_dm: bool) -> Conversation:
        """获取或创建对话"""
    ...
    def add_user_message(self, message_data: Dict) -> Conversation:
        ...
        conversation = self.get_or_create_conversation(converse_id, user_id, user_name, is_dm)
    ...
    def add_assistant_message(self, converse_id: str, content: str, user_id: str = None, user_name: str = None) -> Optional[Conversation]:
        """添加助手回复"""
    ...
    def get_conversation_history_for_ai(self, converse_id: str, max_messages: int = None) -> List[Dict[str, str]]:
        """获取AI可用的对话历史"""
        if converse_id not in self.conversations:
            return []
    ...
    def get_conversation_summary(self, converse_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "updated_at": datetime.fromtimestamp(conversation.updated_at).strftime('%Y-%m-%d %H:%M:%S'),
            "recent_messages": [
                {
                    "role": msg.role,
    ...
    def cleanup_old_conversations(self, max_age_days: int = 30):
        ...
        for converse_id, conversation in list(self.conversations.items()):
            if conversation.updated_at < cutoff_time and conversation.message_count == 0:
                self.delete_conversation(converse_id)
                deleted_count += 1
        
        logger.info(f"清理了 {deleted_count} 个旧对话")
    
    def export_conversation(self, converse_id: str, format: str = 'json') -> Optional[str]:
        """导出对话"""
        if converse_id not in self.conversations:
            return None
        ...
        if format == 'json':
            ...
            lines.append(f"创建时间: {datetime.fromtimestamp(conversation.created_at).strftime('%Y-%m-%d %H:%M:%S')}")
            lines.append(f"更新时间: {datetime.fromtimestamp(conversation.updated_at).strftime('%Y-%m-%d %H:%M:%S')}")
            lines.append("=" * 50)
