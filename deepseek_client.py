...
class DeepSeekClient:
    ...
    def chat(self,
        ...
        try:
            ...
            # 构建消息历史（使用对话管理器）
            messages = self._build_messages_with_history(message, conversation_id, system_prompt)
    ...
    def _build_messages_with_history(self,
        ...
        # 添加历史消息（从对话管理器获取）
        if conversation_id and config.conversation.enable_history:
            history = conversation_manager.get_conversation_history_for_ai(conversation_id)
            if history:
    ...
    def _build_messages(self,
                       ...
                       system_prompt: Optional[str]) -> List[Dict[str, str]]:
        """构建消息列表（兼容旧版本）"""
        return self._build_messages_with_history(message, conversation_id, system_prompt)
